...
\paragraph{Clase base \textit{ImageLoader}} \label{par:8_3_2_1_image_loader}
...
\paragraph{Loaders implementados} \label{par:8_3_2_2_loaders}
...
\paragraph{Módulo de orquestación} \label{par:8_3_2_3_modulo_orquestacion}
...
el \autoref{sec:appendix_C_raw_formats}.}
...
\paragraph{Clase base \textit{ImageOperation}} \label{par:8_3_3_1_image_operation}
...
\begin{itemize}[noitemsep]
    ...
    \begin{enumerate}
        ...
        \item Garantiza que el tensor de salida sea contíguo en memoria.
...
\paragraph{Sistema de registro} \label{par:8_3_3_2_sistema_registro}
...
\paragraph{Operaciones implementadas} \label{par:8_3_3_3_operaciones_implementadas}
...
orquesatación de la interacción de entra la interfaz gráfica, los \textit{loaders} y el motor de 
...
\paragraph{Gestión de la imagen y su estado} \label{par:8_3_4_1_gestion_imagen_estado}
...
\begin{enumerate}[noitemsep]
    ...
    \item Mantiene una copia de referencia de la imagen original para permitir reseteos.
\end{enumerate}
...
\paragraph{Gestión del \textit{pipeline} de operaciones} \label{par:8_3_4_2_gestion_pipeline_operaciones}
...
\paragraph{Sistema de \textit{snaphots}} \label{par:8_3_4_3_sistema_snapshots}
...
\paragraph{Actualización de la interfaz} \label{par:8_3_4_4_actualizacion_interfaz}
...
bien definida. En la \autoref{fig:28_gui_no_image_loaded_overview} y la \autoref{fig:29_gui_image_loaded_overview}
se muestra una visión general.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8425\textwidth]{sections/8/img/gui_no_image_loaded.png}
    \caption{Visión general de la interfaz gráfica (sin imagen cargada)}
    \label{fig:28_gui_no_image_loaded_overview}
\end{figure}

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8425\textwidth]{sections/8/img/gui_image_loaded.png}
    \caption{Visión general de la interfaz gráfica (con imagen cargada)}
    \label{fig:29_gui_image_loaded_overview}
\end{figure}
...
\paragraph{Panel izquierdo (\textit{LeftPanel})} \label{par:8_3_5_1_left_panel}
El panel izquierdo es el encargado de gestionar las principales acciones que se pueden realizar desde 
la interfaz gráfica. Actúa como centro de control. En la \autoref{fig:30_gui_left_panel_overview} se 
muestra su disposición respecto al resto de la interfaz.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8425\textwidth]{sections/8/img/gui_left_panel.png}
    \caption{Panel izquierdo (\textit{LeftPanel})}
    \label{fig:30_gui_left_panel_overview}
\end{figure}
...
\begin{minipage}[t]{0.49\textwidth}
    ...
    \begin{itemize}[noitemsep]
        \item \textbf{Histograma RGB}. Muestra la distribución de valores de píxeles para cada canal de color.
        \item \textbf{Perfiles centrales de intensidad}. Representa la intensidad de los perfiles vertical 
        y horizontal que atraviesan el centro de la imagen.
    \end{itemize}
    En la \autoref{fig:31_left_panel} se muestran dichos gráficos.
\end{minipage}\hfill
\begin{minipage}[t]{0.49\textwidth}
    \vspace{0pt}
    \centering
    \includegraphics[width=0.8\linewidth]{sections/8/img/left_panel_plots.png}
    \captionsetup{justification=centering}
    \captionof{figure}{Gráficos de información \\ de la imagen}
    \label{fig:31_left_panel}
\end{minipage}
...
\begin{minipage}[t]{0.49\textwidth}
    ...
    todas las operaciones de procesamiento aplicadas. Dichos botones se muestran en la \autoref{fig:32_save_reset_buttons}.
    \vspace{1em}

    Tras estos botones, se renderiza uno de los componentes más importantes de la interfaz, el \textit{selector de operaciones}
    (\autoref{fig:33_pipeline_component}) para el \textit{pipeline} de procesamiento. Este componente permite añadir y eliminar 
    ...
    posteriormente. Además, en la parte inferior izquierda del componente, se rederiza un botón de tipo \textit{toggle} [CPU/GPU] que permite 
    cambiar el dispositivo de procesamiento en el que se ejecutan las operaciones, mostrando el dispositivo seleccionado.
    \vspace{1em}

    Finalmente, en la parte inferior del panel izquierdo, se renderizan dos botones adicionales (\autoref{fig:34_op_profile_save_image_buttons}).
...
\begin{minipage}[t]{0.49\textwidth}
    \vspace{0pt}
    \centering
    \includegraphics[width=0.85\textwidth]{sections/8/img/save_reset_buttons.png}
    \captionsetup{justification=centering}
    \captionof{figure}{Botones de guardado y \\ reinicio}
    \label{fig:32_save_reset_buttons}
    \vspace{2.25em}
    \includegraphics[width=0.85\linewidth]{sections/8/img/pipeline_component.png}
    \captionsetup{justification=centering}
    \captionof{figure}{Selector de operaciones}
    \label{fig:33_pipeline_component}
    \vspace{2.25em}
    \includegraphics[width=0.85\linewidth]{sections/8/img/op_profile_save_image_buttons.png}
    \captionsetup{justification=centering}
    \captionof{figure}{Botones para generar el \\ perfil de ejecución de operaciones y \\ para guardar la imagen procesada}
    \label{fig:34_op_profile_save_image_buttons}
\end{minipage}

\newpage

\paragraph{Visor central (\textit{ImageViewer})} \label{par:8_3_5_2_image_viewer}
El visor central es el componente principal de toda la interfaz. Es el encargado de mostrar la imagen cargada y procesada, actualizdándose 
...
el propio procesamiento. En la \autoref{fig:35_gui_image_viewer_overview} se muestra su disposición respecto al resto de la interfaz.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8425\textwidth]{sections/8/img/gui_image_viewer.png}
    \caption{Visor central (\textit{ImageViewer})}
    \label{fig:35_gui_image_viewer_overview}
\end{figure}
...
se puede observar en la \autoref{fig:36_image_info_tab_pane}.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9425\textwidth]{sections/8/img/image_info_tab_pane.png}
    \caption{Sección de \textit{tabs} del visor central}
    \label{fig:36_image_info_tab_pane}
\end{figure}
...
En la \autoref{fig:37_logger_tab_pane} se muestra un ejemplo de esta pestaña, que refleja el registro de acciones realizadas para la toma de las capturas de pantalla anteriores:
Carga de imagen $\rightarrow$ Aplicación de \textit{Debyaer} $\rightarrow$ Aplicación de Normalización mín. máx. $\rightarrow$ Aplicación de Ecualización de Histograma $\rightarrow$ Aplicación de Ajuste \textit{Gamma}.
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.98\textwidth]{sections/8/img/logger_tab_pane.png}
    \caption{Pestaña de \textit{Logger} del visor central}
    \label{fig:37_logger_tab_pane}
\end{figure}

Además, tras la aplicación de cada operación, se muestra el tiempo de ejecución de la misma en milisegundos, que permite obtener una primera referencia sobre el rendimiento de cada operación aplicada.
...
que puede ser de gran utilidad. En la \autoref{fig:38_logger_cuda_cpu} se muestra un ejemplo de esta funcionalidad, donde se observa la aplicación de ciertas operaciones en ambos dispositvos 
de ejecución y los tiempos correspondientes.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.98\textwidth]{sections/8/img/logger_cuda_cpu.png}
    \caption{Cambio de dispositivo de procesamiento y referencia de tiempos de ejecución}
    \label{fig:38_logger_cuda_cpu}
\end{figure}
...
las GPUs disponibles en el sistema, como puede observarse en la \autoref{fig:39_device_info_tab_pane}.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.98\textwidth]{sections/8/img/device_info_tab_pane.png}
    \caption{Pestaña de \textit{Device Info} del visor central}
    \label{fig:39_device_info_tab_pane}
\end{figure}

\newpage

\paragraph{Panel derecho (\textit{RightPanel})} \label{par:8_3_5_3_right_panel}
...
un control específico para dicha operación en el panel derecho. En la \autoref{fig:40_gui_right_panel_overview} 
se muestra su disposición respecto al resto de la interfaz, así como un ejemplo de los controles renderizados 
para un \textit{pipeline} en específico.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8425\textwidth]{sections/8/img/gui_right_panel.png}
    \caption{Panel derecho (\textit{RightPanel})}
    \label{fig:40_gui_right_panel_overview}
\end{figure}
...
selección entre diferentes algoritmos. Dicho control, muetra un simple menú desplegable de tipo \textit{dropdown} con las opciones 
disponibles para cada operación, como se muestra en la \autoref{fig:41_op_controls_method_selection}.

\begin{figure}[h!]
    \centering
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=0.925\textwidth]{sections/8/img/debayer_op_control.png}
        \caption{Control para operación \textit{Debayer}}
        \label{fig:41_a_debayer_op_control}
    \end{subfigure}\hfill
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=0.925\textwidth]{sections/8/img/white_balance_op_control.png}
        \caption{Control para operación \textit{White Balance}}
        \label{fig:41_b_white_balance_op_control}
    \end{subfigure}
    \caption{Controles para operaciones con selección de método o algoritmo}
    \label{fig:41_op_controls_method_selection}
\end{figure}
...
actualización inmediata de la imagen en el visor central, mostrando el resultado del ajuste. En la \autoref{fig:42_op_controls_filters}
...
\begin{figure}[h!]
    ...
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=0.925\textwidth]{sections/8/img/affine_intensity_transformation_op_control.png}
        \caption{Control para Ajuste de Intensidad Afín}
        \label{fig:42_a_affine_intensity_transformation_op_control}
    \end{subfigure}\hfill
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=0.925\textwidth]{sections/8/img/clahe_op_control.png}
        \caption{Control para \textit{CLAHE}}
        \label{fig:42_b_clahe_op_control}
    \end{subfigure}
    \\[1em]
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=0.925\textwidth]{sections/8/img/gamma_adjustment_op_control.png}
        \caption{Control para Ajuste \textit{Gamma}}
        \label{fig:42_c_gamma_adjustment_op_control}
    \end{subfigure}\hfill
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=0.925\textwidth]{sections/8/img/sigmoid_contrast_op_control.png}
        \caption{Control para Ajuste de Contraste mediante Sigmoide}
        \label{fig:42_d_sigmoid_contrast_op_control}
    \end{subfigure}
    \\[1em]
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=0.925\textwidth]{sections/8/img/unsharp_masking_op_control.png}
        \caption{Control para \textit{Unsharp Masking}}
        \label{fig:42_e_unsharp_masking_op_control}
    \end{subfigure}\hfill
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=0.925\textwidth]{sections/8/img/brightness_adjustment_op_control.png}
        \caption{Control para Ajuste de Brillo}
        \label{fig:42_f_brightness_adjustment_op_control}
    \end{subfigure}
    \\[1em]
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=0.925\textwidth]{sections/8/img/gaussian_filter_op_control.png}
        \caption{Control para Filtro Gaussiano}
        \label{fig:42_g_gaussian_filter_op_control}
    \end{subfigure}\hfill
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=0.925\textwidth]{sections/8/img/mean_contrast_adjustment_op_control.png}
        \caption{Control para Ajuste de Contraste}
        \label{fig:42_h_mean_contrast_adjustment_op_control}
    \end{subfigure}
    \\[1em]
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=0.925\textwidth]{sections/8/img/median_filter_op_control.png}
        \caption{Control para Filtro de Mediana}
        \label{fig:42_i_median_filter_op_control}
    \end{subfigure}\hfill
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=0.925\textwidth]{sections/8/img/rotate_op_control.png}
        \caption{Control para Rotación de imagen}
        \label{fig:42_j_rotate_op_control}
    \end{subfigure}
    \caption{Controles para operaciones con parámetros numéricos ajustables}
    \label{fig:42_op_controls_filters}
\end{figure}
...
\autoref{fig:43_numeric_spin_op_controls}. Estos controles renderizan dos elementos de tipo \textit{spinbox} para cada 
...
\begin{figure}[h!]
    ...
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=0.925\textwidth]{sections/8/img/min_max_with_params_op_control.png}
        \caption{Control para Normalización con parámetros}
        \label{fig:43_a_min_max_with_params_op_control}
    \end{subfigure}
    \\[1.5em]
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=0.925\textwidth]{sections/8/img/min_max_percentile_op_control.png}
        \caption{Control para Normalización con percentiles}
        \label{fig:43_b_min_max_percentile_op_control}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=0.925\textwidth]{sections/8/img/resize_op_control.png}
        \caption{Control para Redimensionamiento}
        \label{fig:43_c_resize_op_control}
    \end{subfigure}
    \caption{Controles para operaciones con parámetros numéricos ajustables (continuación)}
    \label{fig:43_numeric_spin_op_controls}
\end{figure}
...
horizontal o vertical). Como puede observarse en la \autoref{fig:44_flip_op_control}, este control renderiza un \textit{radio button}
que permite seleccionar entre ambas opciones.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.45\textwidth]{sections/8/img/flip_op_control.png}
    \caption{Control para Volteo de imagen}
    \label{fig:44_flip_op_control}
\end{figure}
...
la intensidad. Para manejar esta operación, se desarrolla el control mostrado en la \autoref{fig:45_light_compensation_op_control}, 
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.45\textwidth]{sections/8/img/light_compensation_op_control.png}
    \caption{Control para Compensación de Iluminación}
    \label{fig:45_light_compensation_op_control}
\end{figure}
...
informativo que indica que no requiere ningún parámetro, como se muestra en la \autoref{fig:46_op_controls_no_param}.

\begin{figure}[h!]
    \centering
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=0.925\textwidth]{sections/8/img/no_param_op_control/color_to_grayscale_op_control.png}
        \caption{Control para Conversión a Escala de Grises}
        \label{fig:46_a_color_to_grayscale_op_control}
    \end{subfigure}\hfill
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=0.925\textwidth]{sections/8/img/no_param_op_control/grayscale_to_color_op_control.png}
        \caption{Control para Conversión a Color}
        \label{fig:46_b_grayscale_to_color_op_control}
    \end{subfigure}
    \\[1em]
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=0.925\textwidth]{sections/8/img/no_param_op_control/histogram_equalization_op_control.png}
        \caption{Control para Ecualización de Histograma}
        \label{fig:46_c_histogram_equalization_op_control}
    \end{subfigure}\hfill
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=0.925\textwidth]{sections/8/img/no_param_op_control/hsv_to_rgb_op_control.png}
        \caption{Control para Conversión HSV a RGB}
        \label{fig:46_d_hsv_to_rgb_op_control}
    \end{subfigure}
    \\[1em]
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=0.925\textwidth]{sections/8/img/no_param_op_control/rgb_to_hsv_op_control.png}
        \caption{Control para Conversión RGB a HSV}
        \label{fig:46_e_rgb_to_hsv_op_control}
    \end{subfigure}\hfill
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=0.925\textwidth]{sections/8/img/no_param_op_control/min_max_normalization_op_control.png}
        \caption{Control para Normalización mín. máx. \footnotemark}
        \label{fig:46_f_min_max_normalization_op_control}
    \end{subfigure}
    \\[1em]
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=0.925\textwidth]{sections/8/img/no_param_op_control/rgb8_to_real_op_control.png}
        \caption{Control para Conversión RGB8 a Real}
        \label{fig:46_g_rgb8_to_real_op_control}
    \end{subfigure}\hfill
    \begin{subfigure}{0.49\textwidth}
        \centering
        \includegraphics[width=0.925\textwidth]{sections/8/img/no_param_op_control/real_to_rgb8_op_control.png}
        \caption{Control para Conversión Real a RGB8}
        \label{fig:46_h_real_to_rgb8_op_control}
    \end{subfigure}
    \caption{Controles para operaciones sin parámetros}
    \label{fig:46_op_controls_no_param}
\end{figure}

\footnotetext{La operación de Normalización mín. máx. (\autoref{fig:46_f_min_max_normalization_op_control}) sin parámetros se
...
mínimo y máximo, lo que se corresponde con la operación mostrada en la \autoref{fig:43_a_min_max_with_params_op_control}.}
...
\paragraph{Arquitectura interna} \label{par:8_3_6_1_arquitectura_interna_benchmark}
...
\begin{itemize}[noitemsep]
    ...
    operación utilizando el registro global de operaciones (\texttt{OPERATION\_REGISTRY}, \autoref{par:8_3_3_2_sistema_registro}).
...
\paragraph{Flujo técnico de ejecución} \label{par:8_3_6_2_flujo_tecnico_benchmark}
...
\paragraph{Comunicación con el resto de la aplicación} \label{par:8_3_6_3_comunicacion_benchmark}
...
\paragraph{Organización y estructura} \label{par:8_3_7_1_organizacion_estructura_tests}
...
\paragraph{Infraestructura de \textit{fixtures}} \label{par:8_3_7_2_infraestructura_fixtures}
El módulo \texttt{tests/conftest.py} define un conjunto de \textit{fixtures} compartidas que
proporcionan utilidades reutilizables a toda la \textit{suite}:

\begin{itemize}[noitemsep]
    \item \texttt{\textbf{base\_tensor}} $\rightarrow$ Genera tensores sintéticos de prueba con forma
    [C, H, W] configurable y tipo de dato especificado (float o int). Permite crear entradas de test
    reproducibles de forma programática.
    \item \texttt{\textbf{assert\_tensors}} $\rightarrow$ Compara dos tensores verificando igualdad de
    forma, tipo de dato y valores dentro de una tolerancia configurable. Facilita la validación de
    resultados numéricos con margen de error controlado.
    \item \texttt{\textbf{assert\_channels}} $\rightarrow$ Verifica que un tensor de imagen tenga el
    número esperado de canales en la dimensión correspondiente. Útil para validar transformaciones
    que alteran la dimensionalidad (como conversiones de color).
\end{itemize}

\paragraph{Tipología de pruebas} \label{par:8_3_7_3_tipologia_pruebas}
...
comportamiento con entradas atípicas o validación de propiedades específicas. También, en escenarios 
más concreto, se ejecutan pruebas paramétricas utilizando la anotación \textit{@pytest.mark.parametrize}
para evaluar múltiples combinaciones de parámetros de forma sistemática.
