...
def enhance_contrast_torch(image: Image, gain: float, cutoff: float = 0.5) -> None:
    ...
    if cutoff < 0:
        ...
        estimated_cutoff: Tensor = torch.mean(image.original_tensor).to(
            device=image.tensor.device, non_blocking=True
        )
        t_cutoff = torch.clip(
    ...
    imgf: Tensor = torch.empty_like(image.original_tensor).to(
    ...
    imgf = 1 / (1 + torch.exp(gain * (t_cutoff - image.original_tensor)))
